...
```
Without cstore:
  project-a/vendor/laravel/framework/  ← full copy (20MB)
  project-b/vendor/laravel/framework/  ← full copy (20MB)  [DUPLICATE]
  project-c/vendor/laravel/framework/  ← full copy (20MB)  [DUPLICATE]

With cstore:
  ~/.composer-store/packages/laravel+framework@11.0.0/  ← stored ONCE (20MB)
  project-a/vendor/laravel/framework/  ← hard link (0MB extra)
  project-b/vendor/laravel/framework/  ← hard link (0MB extra)
  project-c/vendor/laravel/framework/  ← hard link (0MB extra)
```
...
~/.composer-store/
  packages/
    laravel+framework@11.0.0/     ← package key format: vendor+name@version
      src/
      composer.json
      .cstore-complete             ← marker: download was complete (not partial)
    filament+filament@3.2.0/
  metadata/
    index.json                    ← future: registry cache

project/vendor/
  laravel/
    framework/
      .cstore-link                ← marker: points to store path (for prune scanning)
      src/                        ← hard linked files from store
  composer/                       ← NEVER managed by cstore, always per-project
    autoload_classmap.php
    autoload_psr4.php
    ...
  autoload.php                    ← per-project
```
...
```
bin/
  cstore                          ← CLI entry point (PHP script)
src/
  Application.php                 ← Symfony Console bootstrap, registers all commands
  Commands/
    InstallCommand.php            ← `cstore install` — main workflow
    StatusCommand.php             ← `cstore status` — store stats
    PruneCommand.php              ← `cstore prune` — remove unused packages
  Store/
    GlobalStore.php               ← manages ~/.composer-store directory
    PackageDownloader.php         ← downloads packages from dist URLs into store
  Linker/
    VendorLinker.php              ← hard links store packages into project vendor/
    AutoloaderGenerator.php       ← shells out to `composer dump-autoload`
  Parser/
    LockFileParser.php            ← parses composer.lock, extracts package metadata
tests/
  (PHPUnit tests — Phase 3)
```
...
### Completion Markers
- `.cstore-complete` written after successful download — prevents partial downloads being used
- `.cstore-link` written in `vendor/pkg/` — contains store path, used by `prune --scan`

### Autoloader Strategy (Phase 1)
- Shells out to `composer dump-autoload` after linking
- This is intentional — reimplementing Composer's autoload generation is complex
...
2. For each package: check if in store → sync if not (`zip`/`tar`/`tgz`/`path`) → hard link into `vendor/`
...
| Phase | Status | Goal |
|-------|--------|------|
| 1 | ✅ Complete | CLI MVP — `install`, `status`, `prune` |
| 2 | ✅ Complete | Composer Plugin — transparent `composer install` integration |
| 3 | ✅ Complete | Edge cases — post-install scripts, integrity checks, PHPUnit suite |
...
- Windows not supported (no hard links via `link()`) — Phase 4
- No parallel downloads — packages downloaded sequentially
